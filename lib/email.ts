...
// 邮件配色与网站主题保持一致 (emerald-600 / emerald-700)，
// 便于用户在收件箱中也能感受到同一品牌色。
const BRAND_COLORS = {
  // 主品牌色 (按钮 / 标题渐变 / 强调)
  primary: '#059669',
  // 深一档的渐变色 (用于 135° 渐变末端 & hover)
  primaryDark: '#047857',
  // 主色淡化背景 (用于信息卡片背景)
  primaryLight: '#ecfdf5',
  // 整体页面背景
  background: '#f8fafc',
  // 兜底的深色背景
  backgroundDark: '#0c0f14',
  // 邮件正文主文字色
  text: '#0f172a',
  // 次要 / 副文字色
  muted: '#64748b',
  // 强调色（用于重要提示区块），沿用 emerald 系列
  accent: '#10b981',
  accentLight: '#d1fae5',
}
  ...
  return `
    ...
    <body style="margin: 0; padding: 0; background-color: ${colors.background}; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <!-- Main Content -->
        <div style="background: white; padding: 40px; border-radius: 16px; margin-bottom: 30px; box-shadow: 0 4px 20px rgba(5, 150, 105, 0.08); border: 1px solid ${colors.accentLight};">
          ...
          <!-- Fallback Link -->
          <div style="background: ${colors.primaryLight}; padding: 20px; border-radius: 8px; border-left: 4px solid ${colors.primary};">
        ...
        <div style="text-align: center; color: ${colors.muted}; font-size: 13px; line-height: 1.6;">
          ...
          <!-- Branding -->
          <div style="margin-top: 24px; padding-top: 20px; border-top: 1px solid ${colors.accentLight};">
...
export async function sendWithdrawRequestAdminEmail(params: {
  ...
  const content = `
    <div style="background: ${colors.primaryLight}; padding: 24px; border-radius: 12px; margin: 24px 0; border: 1px solid ${colors.accentLight};">
...
const reminderCopy = {
  '7d': {
    'zh-CN': {
      ...
      body1: '感谢您使用 FileMarkd。为避免您的服务中断,请考虑在到期前续订。',
      body2: '当前订阅计划:',
      body3: '您可访问下方页面查看续订方案并完成续订,服务将在到期后自动顺延。',
      cta: '前往续订',
      footer1: '本邮件由 FileMarkd 自动发送,仅与您的订阅状态相关。',
    ...
    en: {
      ...
      body1: 'Thanks for using FileMarkd. To avoid any service interruption, please renew before it expires.',
      body2: 'Current plan:',
      ...
      cta: 'Renew Now',
      footer1: 'This is an automated message from FileMarkd regarding your subscription.',
    ...
    ja: {
      ...
      body1: 'FileMarkdをご利用いただきありがとうございます。サービス中断を避けるため、有効期限前に更新をご検討ください。',
      body2: '現在の購読プラン:',
    ...
    ko: {
      ...
      body1: 'FileMarkd를 이용해 주셔서 감사합니다. 서비스 중단을 피하시려면 만료 전에 갱신해 주세요.',
      body2: '현재 구독 플랜:',
    ...
    'zh-TW': {
      ...
      heading: '訂閱即將到期',
      greeting: (name: string) => `${name},您好:`,
      body1: '感謝您使用 FileMarkd。為避免您的服務中斷,請考慮在到期前續訂。',
      body2: '當前訂閱計劃:',
      body3: '請前往下方頁面查看續訂方案並完成續訂,服務將在到期後自動順延。',
      cta: '前往續訂',
      footer1: '本郵件由 FileMarkd 自動發送,僅與您的訂閱狀態相關。',
  ...
  '3d': {
    'zh-CN': {
      ...
      heading: '订阅即将到期',
      greeting: (name: string) => `${name},您好:`,
      body1: '再过 3 天您的订阅即将到期。为了不间断使用 FileMarkd 的全部功能,请尽快续订。',
      body2: '当前订阅计划:',
    ...
    en: {
      ...
      greeting: (name: string) => `Hi ${name},`,
      body1: 'Your subscription will end in 3 days. To keep using all FileMarkd features without interruption, please renew now.',
      body2: 'Current plan:',
    ...
    ja: {
      ...
      greeting: (name: string) => `${name} 様`,
      body1: 'あと3日でサブスクリプションが到期します。FileMarkdの全機能を中断なくご利用いただくため、今すぐ更新してください。',
      body2: '現在の購読プラン:',
    ...
    ko: {
      ...
      greeting: (name: string) => `${name} 님, 안녕하세요.`,
      body1: '3일 후 구독이 만료됩니다. FileMarkd의 모든 기능을 중단 없이 이용하시려면 지금 갱신해 주세요.',
      body2: '현재 구독 플랜:',
    ...
    'zh-TW': {
      ...
      heading: '訂閱即將到期',
      greeting: (name: string) => `${name},您好:`,
      body1: '再過 3 天您的訂閱即將到期。為了不中斷使用 FileMarkd 的全部功能,請儘快續訂。',
      body2: '當前訂閱計劃:',
      ...
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: white; padding: 40px; border-radius: 16px; margin-bottom: 30px; box-shadow: 0 4px 20px rgba(5, 150, 105, 0.08); border: 1px solid ${colors.accentLight};">
          ...
          <div style="text-align: center; margin: 32px 0;">
            <a href="${renewUrl}" style="background: linear-gradient(135deg, ${colors.primary} 0%, ${colors.primaryDark} 100%); color: white; padding: 14px 36px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; display: inline-block; box-shadow: 0 8px 24px rgba(5, 150, 105, 0.3);">
              ${copy.cta}
            </a>
          </div>
        </div>
        ...
        <div style="text-align: center; color: ${colors.muted}; font-size: 13px; line-height: 1.6;">
          ...
          <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid ${colors.accentLight};">
...
export const reengagementCopy: Record<ReengagementBucket, Record<EmailLocale, ReengagementCopy>> = {
  warm: {
    'zh-CN': {
      subject: '好久不见，FileMarkd 有新功能等你体验',
      preview: '您有一段时间没来了，我们新增了很多实用的功能，快来看看吧！',
      heading: '想念你，来看新功能',
      greeting: (name) => `${name}，您好：`,
      body1: '您有一段时间没使用 FileMarkd 了。期间我们上线了不少新功能和改进，希望您会喜欢。',
      body2: '感谢您的陪伴，期待再次看到您的活跃身影。',
      cta: '立即体验新功能',
      footer1: '本邮件由 FileMarkd 自动发送。',
      footer2: '如果您不想再收到此类邮件，可以取消订阅。',
      footer3: '取消邮件订阅',
    },
    en: {
      subject: "Been a while? New features await you on FileMarkd",
      ...
      body2: "Thanks for being part of FileMarkd. We'd love to see you back!",
      cta: 'Try New Features',
      footer1: 'This is an automated email from FileMarkd.',
      footer2: 'If you prefer not to receive emails like this, you can unsubscribe.',
      footer3: 'Unsubscribe',
    },
    ja: {
      subject: 'しばらくぶりのご挨拶 — FileMarkdに新機能登場',
      preview: '最後にご利用顶いてから时日が経ちました。新しい機能が牺い上がっています！',
      heading: 'おかえりなさい — 新機能が登場',
      greeting: (name) => `${name} 様`,
      body1: '最後にご利用顶いてから時間が空きました。この間に多くの新機能・改善を追加しました。',
      body2: 'FileMarkdを利用いただきありがとうございます。またのご登录をお待ちしています。',
      cta: '新機能を見る',
      footer1: '本メールは FileMarkd から自動送信されています。',
      footer2: '此类メールの受信をご希望でない場合は購読解除できます。',
      footer3: '購読解除',
    },
    ko: {
      subject: '오랜만이에요 — FileMarkd에 새로운 기능이 탄생했어요',
      ...
      body2: 'FileMarkd를 이용해 주셔서 감사합니다. 다시 뵐 수 있기를 기대할게요!',
      cta: '새로운 기능 보기',
      footer1: '본 메일은 FileMarkd에서 자동 발송됩니다。',
      footer2: '此类 메일을 더 이상 원치 않으시면 구독을 해지할 수 있습니다。',
      footer3: '구독 해지',
    },
    'zh-TW': {
      subject: '好久不見，FileMarkd 有新功能等你體驗',
      preview: '您有一段時間沒來了，我們新增了很多實用的功能，快來看看吧！',
      heading: '想念你，來看新功能',
      greeting: (name) => `${name}，您好：`,
      body1: '您有一段時間沒使用 FileMarkd 了。期間我們上線了不少新功能和改進，希望您會喜歡。',
      body2: '感謝您的陪伴，期待再次看到您的活躍身影。',
      cta: '立即體驗新功能',
      footer1: '本郵件由 FileMarkd 自動發送。',
  ...
  dormant: {
    'zh-CN': {
      subject: '您有专属福利待领取，FileMarkd 等您回来',
      preview: '我们注意到您一段时间没访问了，这里有一份专为您准备的特别福利。',
      heading: '我们想念你，专属福利送你',
      greeting: (name) => `${name}，您好：`,
      body1: '我们注意到您已经有一段时间没有使用 FileMarkd 了。我们想念您，也为您准备了一份专属福利作为感谢。',
    ...
    en: {
      subject: "We miss you — a special offer just for you on FileMarkd",
      ...
      greeting: (name) => `Hi ${name},`,
      body1: "It's been a while since you last visited FileMarkd. We miss you, and we've prepared a special offer as a token of our appreciation.",
    ...
    ja: {
      ...
      greeting: (name) => `${name} 様`,
      body1: 'しばらく FileMarkd をご利用いただけていませんが、私たちがあなたのために特別な 혜택を用意しました。',
    ...
    ko: {
      subject: '환영합니다 — FileMarkd가 당신을 위해 특별 혜택을 준비했어요',
      preview: '오랫동안 방문하지 않으셨지만, 당신만을 위한 특별한 혜택을 준비했답니다。',
      heading: '다시 오신 것을 환영합니다 — 특별 혜택',
      greeting: (name) => `${name} 님, 안녕하세요`,
      body1: '오랫동안 FileMarkd를 방문하지 않으셨네요。당신을 위한 특별 혜택을 준비했답니다。',
    ...
    'zh-TW': {
      subject: '您有專屬福利待領取，FileMarkd 等您回來',
      preview: '我們注意到您一段時間沒訪問了，這裡有一份專為您準備的特別福利。',
      heading: '我們想念你，專屬福利送你',
      greeting: (name) => `${name}，您好：`,
      body1: '我們注意到您已經有一段時間沒有使用 FileMarkd 了。我們想念您，也為您準備了一份專屬福利作為感謝。',
  ...
  inactive: {
    'zh-CN': {
      subject: '限时回归优惠，仅剩 3 天 — FileMarkd',
      preview: '我们想念您！现在回来可享受限时回归优惠，仅剩 3 天。',
      heading: '限时 3 天回归优惠',
      greeting: (name) => `${name}，您好：`,
      body1: '我们注意到您已经很久没有使用 FileMarkd 了。为了感谢您曾经的陪伴，我们准备了一个限时回归优惠。',
    ...
    en: {
      subject: "Your exclusive return offer expires in 3 days — FileMarkd",
      preview: "We miss you! Claim your exclusive return offer — only 3 days left.",
      heading: '3-Day Return Offer',
      greeting: (name) => `Hi ${name},`,
      body1: "It's been a while since you last used FileMarkd. As a token of our appreciation for your past support, we'd like to offer you an exclusive return deal.",
    ...
    ja: {
      ...
      heading: '3日間有効な回来了优惠',
      greeting: (name) => `${name} 様`,
      body1: '最後に FileMarkd をご利用顶いてから已经很长时间。我们为您准备了限定的回来了优惠。',
    ...
    ko: {
      ...
      greeting: (name) => `${name} 님, 안녕하세요`,
      body1: '오랫동안 FileMarkd를 방문하지 않으셨네요。당신을 위한 특별한 돌아온 혜택을 준비했어요。',
    ...
    'zh-TW': {
      subject: '限時回歸優惠，僅剩 3 天 — FileMarkd',
      preview: '我們想念您！現在回來可享受限時回歸優惠，僅剩 3 天。',
      heading: '限時 3 天回歸優惠',
      greeting: (name) => `${name}，您好：`,
      body1: '我們注意到您已經很久沒有使用 FileMarkd 了。為了感謝您曾經的陪伴，我們準備了一個限時回歸優惠。',
  ...
  churned: {
    'zh-CN': {
      ...
      heading: '感谢您曾经的陪伴',
      greeting: (name) => `${name}，您好：`,
      body1: '我们注意到您已经很久没有使用 FileMarkd 了。感谢您曾经选择我们，即使您离开了，我们依然保留着您的账户和所有数据。',
      body2: '如果您愿意，我们随时欢迎您的回归。也欢迎您将 FileMarkd 推荐给需要的朋友。',
      cta: '重新激活账户',
      footer1: '本邮件由 FileMarkd 自动发送。',
      footer2: '如果您不想再收到此类邮件，可以取消订阅。',
      footer3: '取消邮件订阅',
    ...
    en: {
      ...
      greeting: (name) => `Hi ${name},`,
      body1: "We've noticed you haven't been active on FileMarkd for a while. Thank you for having been part of our community. Even though you've stepped away, we've kept your account and data safe.",
      body2: "When you're ready, we'd love to welcome you back. And if FileMarkd could help someone you know, feel free to share it with them.",
    ...
    ja: {
      ...
      greeting: (name) => `${name} 様`,
      body1: '長い間 FileMarkd をご爱顧いただきありがとうございます。離れていても、あなたのアカウントとデータは安全に保たれています。',
      body2: 'もしよければ、いつでも。欢迎您をじます。また、FileMarkd がお友達のお役にも立ちそうであれば、ぜひご 추천ください。',
      cta: 'アカウントを再開',
      footer1: '本メールは FileMarkd から自動送信されています。',
      footer2: '此类メールの受信をご希望でない場合は購読解除できます。',
      footer3: '購読解除',
    ...
    ko: {
      ...
      body1: '긴 시간 FileMarkd를 이용해주셔서 감사합니다。떠나셨더라도 계정과 데이터는 안전하게 보관하고 있습니다。',
      body2: '언제든 다시 오시면 기쁠 거예요。또한 FileMarkd가 지인분께 도움이 될 것 같으시다면 언제든 추천해 주세요。',
      cta: '계정 재활성화',
      footer1: '본 메일은 FileMarkd에서 자동 발송됩니다。',
      footer2: '此类 메일을 더 이상 원치 않으시면 구독을 해지할 수 있습니다。',
      footer3: '구독 해지',
    ...
    'zh-TW': {
      ...
      heading: '感謝您曾經的陪伴',
      greeting: (name) => `${name}，您好：`,
      body1: '我們注意到您已經很久沒有使用 FileMarkd 了。感謝您曾經選擇我們，即使您離開了，我們依然保留著您的帳戶和所有資料。',
      body2: '如果您願意，我們隨時歡迎您的回歸。也歡迎您將 FileMarkd 推薦給需要的朋友。',
      cta: '重新啟動帳戶',
      footer1: '本郵件由 FileMarkd 自動發送。',
      footer2: '如果您不想再收到此類郵件，可以取消訂閱。',
      footer3: '取消郵件訂閱',
  ...
  sleeping_paid: {
    'zh-CN': {
      subject: '感谢您曾经的信任，FileMarkd 为您准备了专属回归礼',
      preview: '我们注意到您曾是我们的付费用户，感谢您一路陪伴。为您准备了专属优惠，期待您回来。',
      heading: '欢迎回来，专属优惠等您领取',
      greeting: (name) => `${name}，您好：`,
      body1: '感谢您曾选择 FileMarkd 作为您的工具。我们注意到您已有一段时间没有使用，感谢您曾经的陪伴。',
    ...
    en: {
      ...
      greeting: (name) => `Hi ${name},`,
      body1: "Thank you for being a valued FileMarkd customer. We've missed you and want to welcome you back with an exclusive offer.",
    ...
    ja: {
      ...
      greeting: (name) => `${name} 様`,
      body1: 'かつて FileMarkd をご利用いただき、誠にありがとうございます。ご不在の間、私たちはあなたの復帰を待ち望んでいました。',
    ...
    ko: {
      ...
      greeting: (name) => `${name} 님`,
      body1: '예전에 FileMarkd 를 이용해 주셔서 진심으로 감사드립니다. 당신이 없는 동안 우리는 당신의 복귀를 손꼽아 기다렸어요.',
    ...
    'zh-TW': {
      ...
      greeting: (name) => `${name}，您好：`,
      body1: '感謝您曾選擇 FileMarkd 作為您的工具。我們注意到您已有一段時間沒有使用，感謝您一路的陪伴。',
...
<body style="margin: 0; padding: 0; background-color: ${colors.background}; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: ${colors.primaryLight}; padding: 40px; border-radius: 16px; margin-bottom: 30px; box-shadow: 0 4px 20px rgba(5, 150, 105, 0.08); border: 1px solid ${colors.accentLight};">
      <h2 style="color: ${colors.text}; margin: 0 0 24px 0; text-align: center; font-size: 26px; font-weight: 700;">${escapeHtml(copy.heading)}</h2>

      <p style="color: ${colors.text}; line-height: 1.7; margin-bottom: 16px; font-size: 16px;">${escapeHtml(copy.greeting(safeName))}</p>
      <p style="color: ${colors.text}; line-height: 1.7; margin-bottom: 16px; font-size: 16px;">${escapeHtml(copy.body1)}</p>
      <p style="color: ${colors.muted}; line-height: 1.7; margin-bottom: 32px; font-size: 16px;">${escapeHtml(copy.body2)}</p>

      <div style="text-align: center; margin: 32px 0;">
        <a href="${escapeHtml(ctaUrl)}" style="background: linear-gradient(135deg, ${colors.primary} 0%, ${colors.primaryDark} 100%); color: white; padding: 14px 36px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; display: inline-block; box-shadow: 0 8px 24px rgba(5, 150, 105, 0.3);">
          ${escapeHtml(copy.cta)}
        </a>
      </div>
    </div>
    ...
    <div style="text-align: center; padding: 0 20px;">
      <p style="color: ${colors.muted}; font-size: 13px; line-height: 1.6; margin-bottom: 8px;">${escapeHtml(copy.footer1)}</p>
      <p style="color: ${colors.muted}; font-size: 13px; line-height: 1.6; margin-bottom: 8px;">${escapeHtml(copy.footer2)}</p>
      <p style="margin-top: 16px;">
        <a href="${escapeHtml(unsubscribeUrl)}" style="color: ${colors.muted}; font-size: 13px; text-decoration: underline;">${escapeHtml(copy.footer3)}</a>
      </p>
    </div>
  </div>
</body>
